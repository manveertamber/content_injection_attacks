...
for dataset in tqdm(datasets):
    ...
    output_filepath = f'passages/keyword_injection_general_passages_{dataset}.jsonl'
    ...
    for qid, query in tqdm(queries_dict.items()):
        ...
        for trial in range(5):            
            ...
            outputs['num_keyword_repetitions'].append(0)
            ...
            for keyword_repetition in [1, 2, 3]:
                for location in ['start', 'middle', 'end']:
                    attack_id = f"{qid}_randpassage_trial_{trial}_sample_{sample_count}"
                    attack_passage = random_passage
                    for repetition_count in range(keyword_repetition):
                        attack_passage = adversarial_generator.inject_query_keywords_into_passage(attack_passage, query, location, expand=False)
                    outputs['attack_ids'].append(attack_id)
                    outputs['qids'].append(qid)
                    outputs['adversarial_passages'].append(attack_passage)
                    outputs["num_injected_words"].append(len(attack_passage.split()) - num_random_passage_words)
                    outputs['num_keyword_repetitions'].append(keyword_repetition)
                    outputs["num_original_words"].append(num_random_passage_words)    
                    outputs["locations"].append(location)
                    outputs["passage_types"].append('randpassage')

                    attack_id = f"{qid}_randwords_trial_{trial}_sample_{sample_count}"
                    attack_passage = random_words_passage
                    for repetition_count in range(keyword_repetition):
                        attack_passage = adversarial_generator.inject_query_keywords_into_passage(attack_passage, query, location, expand=False)
                    outputs['attack_ids'].append(attack_id)
                    outputs['qids'].append(qid)
